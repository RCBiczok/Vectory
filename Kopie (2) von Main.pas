...
uses
  ...
  Geometry, GLTexture, GLParticles, GLContext, VectorTypes, ExtCtrls, GLCadencer,
  ...
  OrtsLinie, OpenGL12, Registry, MultiButton;
...
//== besondere Lage feststellen
    ...
    If Punkt.Position.Z<>0 then begin
      SetLength(Lage, length(Lage)+1);
      Lage[length(Lage)-1]:='X1';
    end;
    If Punkt.Position.X<>0 then begin
      SetLength(Lage, length(Lage)+1);
      Lage[length(Lage)-1]:='X2';
    end;
    If Punkt.Position.Y<>0 then begin
      SetLength(Lage, length(Lage)+1);
...
procedure AdjustGrid(var Grid: TXYZGrid; ZoomGrade, GridScale: single);
begin
...
procedure AdjustTicks(var TicksDummy: TDummyCube; ZoomGrade, GridScale: single);
var
...
// X1 - Richtung
  ...
  for i := Min to Max do
    With TSpaceText(Ticks.Children[i-Min+1]) do begin
...
//== Neue Gerade erstellen
  With TGerade(Lines.AddNewChild(TGerade)) do begin
    ...
    Richtung:=TCone(AddNewChild(TCone));
...
//== Punkt als Partikel erzeugen
  With TPunkt(Points.CreateParticle) do begin
   ...
   ParamPosX2:=MakeNiceNumber(p2);
   ParamPosX3:=MakeNiceNumber(p3);
   Bezeichnung:=TSpaceText(Points.Children[Points.Count-1].AddNewChild(TSpaceText));                       // *F*
...
//== Initialize Objects
  ...
  With TPunkt(Points.AddNewChild(TPunkt)) do begin
    Radius := 0.05;
    Bezeichnung := TSpaceText.Create(DummyCube1);
    Bezeichnung.Text := 'P';
  ...
  for i:=0 to Length(OldSelectedObjects)-1 do
    If Assigned(OldSelectedObjects[i]) then begin
      If (OldSelectedObjects[i] is TGerade) or (OldSelectedObjects[i] is TStrecke) then begin
        TCylinder(OldSelectedObjects[i]).BottomRadius:= TCylinder(OldSelectedObjects[i]).BottomRadius-0.001;
        TCylinder(OldSelectedObjects[i]).TopRadius:= TCylinder(OldSelectedObjects[i]).TopRadius-0.001;
      end;
...
//== Benennung, falls vorhanden
          ...
          If (SelectedObjects[i] is TGerade) or (SelectedObjects[i] is TStrecke) then begin
            TCylinder(SelectedObjects[i]).BottomRadius:= TCylinder(SelectedObjects[i]).BottomRadius+0.001;
            TCylinder(SelectedObjects[i]).TopRadius:= TCylinder(SelectedObjects[i]).TopRadius+0.001;
          end;
//== oder hell
...
begin
  ...
  for i:=0 to TreeView1.SelectionCount-1 do
    if (TGLNode(TreeView1.Items[i]).LinkedObject is TKugel) or
       (TGLNode(TreeView1.Items[i]).LinkedObject is TStrecke) then begin

      ShowMessage('Kugeln oder Strecken können mit diesem Programm leider nicht geschnitten werden');
      Exit;
    end;
...
begin
  ...
  If TreeView1.SelectionCount = 2 then begin
      If (TreeView1.Selections[0].ImageIndex = 1) and (TreeView1.Selections[1].ImageIndex = 1) then
      ...
      If (TreeView1.Selections[0].ImageIndex = 1) and (TreeView1.Selections[1].ImageIndex = 2) then
        AbstandPunktGeradeBestimmen(Self);
      If (TreeView1.Selections[0].ImageIndex = 2) and (TreeView1.Selections[1].ImageIndex = 1) then
        AbstandGeradePunktBestimmen(Self);
      If (TreeView1.Selections[0].ImageIndex = 1) and (TreeView1.Selections[1].ImageIndex = 3) then
        AbstandPunktEbeneBestimmen(Self);
      If (TreeView1.Selections[0].ImageIndex = 3) and (TreeView1.Selections[1].ImageIndex = 1) then
      ...
      If (TreeView1.Selections[0].ImageIndex = 1) and (TreeView1.Selections[1].ImageIndex = 5) then
