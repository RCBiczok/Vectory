...
const
  ReservedWords: Array [0..2] of string =
  ('sqrt', 'sin', 'cos');
...
begin
  ...
  If (Length(Op)>2) and (Op[1]='{') and (Op[Length(Op)]='}') then begin
    ...
    for i:=1 to Length(ActStr) do
      If not (ActStr[i] in ['0'..'9']) then begin
        Result:='error';
        Exit;
      end;
    Exit;
...
begin
  ...
  if act = 'cos' then
    With Result do begin
      ...
      VarArray[0]:=cos(StrToFloat(op2)/180*pi);
    end;
  if act = 'sin' then
    With Result do begin
      SetLength(VarArray, 1);
      VarArray[0]:=sin(StrToFloat(op2)/180*pi);
    end;
  if act = 'sqrt' then
    With Result do begin
      SetLength(VarArray, 1);
      VarArray[0]:=sqrt(StrToFloat(op2));
    end;
  if act = '^' then
    With Result do begin
      SetLength(VarArray, 1);
      VarArray[0]:=Power(StrToFloat(op1),StrToFloat(op2));
    end;
  if act = '*' then
    With Result do begin
      SetLength(VarArray, 1);
      VarArray[0]:=StrToFloat(op1)*StrToFloat(op2);
    end;
  if act = '/' then
    With Result do begin
      SetLength(VarArray, 1);
...
begin
  ...
  If act = '+' then
    Result.VarArray[0]:=StrToFloat(op2);

  If act = '-' then
    Result.VarArray[0]:= -StrToFloat(op2);

  If act = '*' then
    Result.VarArray[1]:= StrToFloat(op2);

  If act = '/' then
    Result.VarArray[1]:= 1/StrToFloat(op2);
    ...
    With Result do begin
      ...
      If act = '*' then begin
        ...
        for i:=0 to Length(ActPoly.VarArray)-1 do
          VarArray[i]:=ActPoly.VarArray[i]*StrToFloat(op2);
      end;
      If act = '/' then begin
{        OK:=True;
...
{  If act = '/' then begin
  ...
  If act = 'sqrt' then begin
    ...
    If OK then begin
      SetLength(Result.VarArray, 1);
      Result.VarArray[0]:=sqrt(ActPoly.VarArray[0]);
    end;
    Exit;
  end;
  If act = '^' then begin
    ...
    If OK then begin
      SetLength(Result.VarArray, 1);
      Result.VarArray[0]:=Power(StrToFloat(op1), ActPoly.VarArray[0]);
    end;
    Exit;
  end;
  If act = '/' then begin
    ...
    If OK then begin
      SetLength(Result.VarArray, 1);
      Result.VarArray[0]:=StrToFloat(op1)/ActPoly.VarArray[0];
    end;
    Exit;
  end;
  If act = '-' then
   With Result do begin
    ...
    VarArray[0]:=ActPoly.VarArray[0]+StrToFloat(op1);
   end
  else
    ...
    With Result do begin
      ...
      If act = '+' then begin
        if PolyLength > 2 then
          ...
          SetLength(VarArray, 2);
        VarArray[1]:=ActPoly.VarArray[1]+1;
      end;
...
begin
  If Assigned(P1.VarArray) and Assigned(P2.VarArray) and
    ...
    for i:=0 to Length(Small.VarArray)-1 do
      Result.VarArray[i]:=Result.VarArray[i]+Small.VarArray[i];
    ...
    With Result do begin
      ...
      for i:=0 to Length(P2.VarArray)-1 do
        If (P2.VarArray[i]<>0) or (i = 0) then begin
          ...
          for j:=0 to Length(P1.VarArray)-1 do
            Dummy.VarArray[j+i]:=P1.VarArray[j]*P2.VarArray[i];
...
begin
  ...
  If {Assigned(ActPoly1.VarArray) and} Assigned(ActPoly2.VarArray) and
    ...
    With Result do begin
      ...
      If act = '-' then begin
        for i:=0 to Length(ActPoly2.VarArray)-1 do
          ActPoly2.VarArray[i]:=-ActPoly2.VarArray[i];
        Result:=PolynomAdd(ActPoly1, ActPoly2);
      end;
      ...
      If act = '/' then begin
        ...
        If OK then begin
          SetLength(VarArray, 1);
          VarArray[0]:=ActPoly1.VarArray[0]/ActPoly2.VarArray[0];
        end;
      end;
      ...
      If act = 'sqrt' then begin
        ...
        If OK then begin
          SetLength(VarArray, 1);
          VarArray[0]:=sqrt(ActPoly2.VarArray[0]);
        end;
      end;
      ...
      If act = 'sin' then begin
        ...
        If OK then begin
          SetLength(VarArray, 1);
          VarArray[0]:=sin(ActPoly2.VarArray[0]/180*pi);
        end;
      end;
      ...
      If act = 'cos' then begin
        ...
        If OK then begin
          SetLength(VarArray, 1);
          VarArray[0]:=cos(ActPoly2.VarArray[0]/180*pi);
        end;
      end;
    end;
end;
  ...
  If Assigned(ActPoly.VarArray) then begin
    for i:= Length(ActPoly.VarArray)-1 downto 1 do
      if ActPoly.VarArray[i]<>0 then begin
        ...
        if ActPoly.VarArray[i] > 0 then
          if ActPoly.VarArray[i] = 1 then
            Result:= Result + '+' + ActVar + '^'+IntToStr(i)
          else
            Result:= Result + '+' +FloatToStr(ActPoly.VarArray[i])+ ActVar + '^'+IntToStr(i);
    ...
    If ActPoly.VarArray[0] > 0 then
      Result:= Result + '+' + FloatToStr(ActPoly.VarArray[0]);
    If ActPoly.VarArray[0] < 0 then
      Result:= Result + FloatToStr(ActPoly.VarArray[0]);
    If Length(ActPoly.VarArray) = 1 then
      Result:=FloatToStr(ActPoly.VarArray[0])
