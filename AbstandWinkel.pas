...
uses
  Main, VectorGeometry, Math, SchnittBerechnung;
...
begin
  ...
  Result:=Abs(RadToDeg(VectorGeometry.ArcCos(Result)));
...
begin
  ...
  Result:=VectorAngleCosine(v1, v2);
  Result:=RadToDeg(VectorGeometry.ArcCos(Result));
  // der Winkel sollte immer positiv sein
  Result:= abs(Result);
...
begin
  ...
  Result:=VectorAngleCosine(v1, v2);
  Result:=90-RadToDeg(VectorGeometry.ArcCos(Result));
...
begin
  ...
  b:= Kreis.MajorRadius * VectorLength(VectorCrossProduct(n, X))/VectorLength(n)/VectorLength(X);
